...
export default function ThongTinSection() {
  ...
  return (
    <section className="relative overflow-hidden py-10">
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 hidden w-[13%] xl:block">
        <i className="bi bi-calendar2-week absolute left-[30%] top-[12%] text-[52px] text-indigo-300" />
        <span className="absolute left-[55%] top-[22%] h-3 w-3 rotate-45 border-2 border-red-300" />
        <span className="absolute left-[20%] top-[40%] h-2 w-2 rotate-45 border-2 border-indigo-300" />
        <i className="bi bi-clock absolute left-[28%] top-[50%] text-[48px] text-indigo-300" />
        <span className="absolute left-[60%] top-[64%] h-2 w-2 rotate-45 border-2 border-red-300" />
        <i className="bi bi-journal-text absolute left-[35%] top-[76%] text-[42px] text-indigo-300" />
      </div>

      {/* ── Decorative icons RIGHT ── */}
      <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-[13%] xl:block">
        <div className="absolute right-[30%] top-[12%]">
          <i className="bi bi-bell text-[50px] text-indigo-300" />
          <span className="absolute right-0 top-1 h-3 w-3 rounded-full bg-red-400" />
        </div>
        <span className="absolute right-[55%] top-[28%] h-3 w-3 rotate-45 border-2 border-indigo-300" />
        <span className="absolute right-[20%] top-[42%] h-2 w-2 rotate-45 border-2 border-red-300" />
        <i className="bi bi-mortarboard absolute right-[28%] top-[52%] text-[50px] text-indigo-300" />
        <span className="absolute right-[58%] top-[68%] h-2 w-2 rotate-45 border-2 border-indigo-300" />
        <i className="bi bi-clipboard-check absolute right-[32%] top-[76%] text-[44px] text-indigo-300" />
      </div>

      <div className="container relative mx-auto space-y-5 px-4">
        {/* ── Section header ── */}
        <RevealOnScroll delay={0} y={24}>
          <div className="mb-4 flex flex-col items-center text-center">
            <div className="mb-8 flex items-center justify-center gap-4 text-xs font-semibold uppercase tracking-[0.24em] text-slate-700 sm:text-sm">
            ...
            <h2 className="text-[clamp(2.6rem,4vw,4.3rem)] font-black leading-[1.35] tracking-[-0.055em] text-slate-950">
              Thông tin &amp; Lịch học
            </h2>
          </div>
        </RevealOnScroll>
        ...
        <div className="mx-auto grid w-full max-w-4xl grid-cols-1 items-stretch gap-5 lg:grid-cols-2">
          {/* Left – Thời khóa biểu */}
          <RevealOnScroll delay={80} y={34} className="h-full">
            <div className="flex h-full min-h-[520px] flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
              {/* panel header */}
              <div className="flex h-[56px] shrink-0 items-center gap-6 border-b border-gray-100 px-6">
                {[
              ...
              <div className="flex-1 overflow-hidden divide-y divide-gray-50">
                {lichHoc.slice(0, 5).map((item, i) => (
                  <RevealOnScroll key={i} delay={140 + i * 70} y={18}>
                    <a
                      href={item.href}
                      className="group grid min-h-[72px] grid-cols-[1fr_96px] items-center gap-4 px-6 py-4 transition-all duration-300 hover:bg-gray-50"
                    >
                      <div className="min-w-0">
                        <p className="line-clamp-2 text-[15.5px] font-semibold leading-snug text-gray-900 transition-colors group-hover:text-[#4F46E5]">
                          {item.title}
                        </p>
                        <p className="mt-1 text-[12px] text-[#4F46E5]">
                          Chi tiết và Phụ lục ↗
                        </p>
                      </div>
              ...
              <div className="mt-auto shrink-0 border-t border-gray-100 px-6 py-4">
                <a
                  href="/thong-tin/thoi-khoa-bieu"
                  className="text-[13px] font-semibold text-[#4F46E5] transition-opacity hover:opacity-70"
                >
                  Xem tất cả →
                </a>
              </div>
          ...
          <RevealOnScroll delay={180} y={34} className="h-full">
            <div className="flex h-full min-h-[520px] flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
              {/* tab header */}
              <div className="flex h-[56px] shrink-0 items-center gap-6 border-b border-gray-100 px-6">
                {[
                ...
                <div className="flex-1" />
                <a
                  href={rightTab === "lichkiemtra" ? "/thong-tin/lich-kiem-tra" : "/thong-tin/thong-bao"}
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-gray-200 transition-all duration-300 hover:rotate-90 hover:bg-gray-50"
                >
                  <i className="bi bi-plus text-[16px] text-gray-500" />
                </a>
              ...
              {/* items */}
              <div key={rightTab} className="flex-1 overflow-hidden divide-y divide-gray-50">
                {rightItems.slice(0, 5).map((item, i) => (
                  <RevealOnScroll key={`${rightTab}-${i}`} delay={140 + i * 70} y={18}>
                    <a
                      href={item.href}
                      className="group grid min-h-[72px] grid-cols-[1fr_96px] items-center gap-4 px-6 py-4 transition-all duration-300 hover:bg-gray-50"
                    >
                      <div className="min-w-0">
                        ...
                        {/* <span
                          className={`mt-1 inline-block rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${TAG_STYLES[item.tagColor]}`}
                        >
                          {item.tag}
                        </span> */}
                      </div>
                      <div className="w-[96px] shrink-0 text-right">
                        ...
                        <p className={`mt-1 text-[13px] font-semibold ${item.dateRed ? "text-[#EF4444]" : "text-gray-700"}`}>
                          {item.date}
                        </p>
                      </div>
                    </a>
