...
import { NEWS_POSTS, NEWS_CATEGORY_LABEL, type NewsCategory } from "@/data/newsPosts";

const EASE = [0.22, 1, 0.36, 1] as const;
const PAGE_SIZE = 6;

const TAG_TABS: { slug: NewsCategory | "tat-ca"; title: string }[] = [
  { slug: "tat-ca", title: "Tất cả" },
  { slug: "su-kien", title: "Sự kiện" },
  { slug: "hoc-thuat", title: "Học thuật" },
  { slug: "cong-nghe", title: "Công nghệ" },
  { slug: "cong-dong", title: "Cộng đồng" },
];

export default function PostGrid() {
  const [activeSlug, setActiveSlug] = useState<NewsCategory | "tat-ca">("tat-ca");
  const [page, setPage] = useState(1);

  const filtered = activeSlug === "tat-ca"
    ? NEWS_POSTS
    : NEWS_POSTS.filter((p) => p.category === activeSlug);

  const visible = filtered.slice(0, page * PAGE_SIZE);
  const hasMore = visible.length < filtered.length;

  function switchTab(slug: NewsCategory | "tat-ca") {
    setActiveSlug(slug);
    setPage(1);
  }
    ...
    <div className="max-w-[1430px] mx-auto px-[12px] mt-10">
      ...
      {visible.length > 0 ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {visible.map((post, i) => {
              ...
              const label = NEWS_CATEGORY_LABEL[post.category];
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center border-t border-slate-100">
          ...
          <p className="text-slate-500 text-[15.5px] max-w-sm">Chưa có tin tức nào.</p>
        </div>
      )}
    </div>
  );
